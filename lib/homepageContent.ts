...
export const getHomepageFallback = (locale: Locale): HomepageContent => {
    if (locale === 'ja-jp') {
        return {
            ...
            pageDescription: 'SkyBridge Air で路線を検索し、運賃を比較して、次のフライトを予約しましょう。',
            heroHeadline: 'スムーズで安心できる予約体験を。',
            heroDescription: '路線検索から予約管理まで、必要な操作をひとつの画面で分かりやすく案内します。',
    ...
    return {
        ...
        heroDescription: 'Search live routes, review fares, and manage your trip from one streamlined booking experience.',
...
const mapHomepageContent = (doc: PrismicLikeDocument, fallback: HomepageContent): HomepageContent => {
    const data = doc.data ?? {};
    ...
    return {
        ...
        navManageBookingLabel: readText(data.nav_manage_booking_label) ?? fallback.navManageBookingLabel,
...
const getHomepageDocumentForLocale = async (locale: Locale): Promise<PrismicLikeDocument | null> => {
    const client = getClient(locale);

    try {
        const byUid = (await client.getByUID('homepage', 'home', client.withLocale())) as PrismicLikeDocument;
        return byUid;
    } catch {
    ...
    try {
        const byType = (await client.getByType('homepage', client.withLocale({ pageSize: 1 }))) as PrismicTypeQueryResult;
