...
import { fetchStaticFlightById } from '../../../../features/flight-search/services/flightService';
import { Flight } from '../../../../types/flight';
import { Locale, defaultLocale, resolveLocaleFromSegment } from '../../../../lib/i18n/index';
...
type LocalizedParams = { locale: string; id: string };
...
const mapPrismicDocumentToFlight = (doc: PrismicLikeDocument, fallback?: Flight): Flight | null => {
  ...
  const mapped: Flight = {
    ...
    flightNumber:
      readText(data.flight_number) ?? readText(data.flightNumber) ?? fallback?.flightNumber ?? '',
    airline: readText(data.airline) ?? fallback?.airline ?? 'SkyBridge Air',
    originCode:
      readText(data.origin_code) ?? readText(data.originCode) ?? fallback?.originCode ?? '',
    originCity:
      readText(data.origin_city) ?? readText(data.originCity) ?? fallback?.originCity ?? '',
    destinationCode:
      readText(data.destination_code) ??
      readText(data.destinationCode) ??
      fallback?.destinationCode ??
      '',
    destinationCity:
      readText(data.destination_city) ??
      readText(data.destinationCity) ??
      fallback?.destinationCity ??
      '',
    departureTime:
      readText(data.departure_time) ??
      readText(data.departureTime) ??
      fallback?.departureTime ??
      new Date().toISOString(),
    arrivalTime:
      readText(data.arrival_time) ??
      readText(data.arrivalTime) ??
    ...
    cabinClass:
      readText(data.cabin_class) ?? readText(data.cabinClass) ?? fallback?.cabinClass ?? 'Economy',
    ...
    seatsLeft:
      readNumber(data.seats_left) ?? readNumber(data.seatsLeft) ?? fallback?.seatsLeft ?? 0,
...
export async function generateMetadata({ params }: { params: LocalizedParams }): Promise<Metadata> {
  const { id, locale: localeSegment } = params;
  const locale = resolveLocaleFromSegment(localeSegment) ?? defaultLocale;
  const detailPath = `/flights/${encodeURIComponent(id)}`;

  try {
    const flight = await fetchFlightDetail(id, locale);
    return createLocaleMetadata(localeSegment, detailPath, {
    ...
    return createLocaleMetadata(localeSegment, detailPath, {
      title: 'Flight not found | SkyBridge Air',
    });
  }
}

export default async function LocalizedFlightDetailPage({ params }: { params: LocalizedParams }) {
  const { id, locale: localeSegment } = params;
  const locale = resolveLocaleFromSegment(localeSegment);
        ...
        <div className="space-y-6 p-8">
          <div>
            ...
            <p className="mt-3 text-lg text-slate-600">
              Rendered fresh on every request so seat availability, pricing, and status always
          ...
          <dl className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl bg-slate-50 p-4">
              ...
              <dd className="mt-2 font-semibold text-slate-900">
                {formatLongDate(flight.departureTime)} UTC
              </dd>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <dt className="text-sm text-slate-500">Arrival</dt>
              <dd className="mt-2 font-semibold text-slate-900">
                {formatLongDate(flight.arrivalTime)} UTC
              </dd>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
