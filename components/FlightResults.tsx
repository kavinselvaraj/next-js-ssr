...
import { searchFlightsAction } from '../app/flights/actions';
    ...
    if (!hasSearched) {
        return (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-6 py-10 text-center text-slate-500">
                ...
                <p className="mt-1 text-sm">Select your origin, destination, and dates to see available flights.</p>
            </div>
        );
    }
        ...
        return (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-6 py-8 text-center text-amber-800">
                ...
                <p className="mt-1 text-sm">
                    No results for{' '}
                    <span className="font-semibold">{originLabel} → {destinationLabel}</span>.
