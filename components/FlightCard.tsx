        ...
        <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
            ...
            <div className="space-y-4 p-5">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-sky-600">{flight.flightNumber}</p>
                        <h2 className="mt-1 text-2xl font-bold text-slate-900">
                            {flight.originCode} → {flight.destinationCode}
                        </h2>
                        <p className="text-sm text-slate-500">{flight.originCity} to {flight.destinationCity}</p>
                    </div>
                    <span className="rounded-full bg-sky-50 px-3 py-1 text-xs font-semibold text-sky-700">{flight.status}</span>
                </div>
                ...
                <dl className="grid grid-cols-2 gap-3 text-sm text-slate-600">
                    <div className="rounded-xl bg-slate-50 p-3">
                        ...
                        <dd className="mt-1 font-semibold text-slate-900">{formatFlightTime(flight.departureTime)} UTC</dd>
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3">
                        <dt className="text-xs uppercase tracking-wide text-slate-400">Arrival</dt>
                        <dd className="mt-1 font-semibold text-slate-900">{formatFlightTime(flight.arrivalTime)} UTC</dd>
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3">
